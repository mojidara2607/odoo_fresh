  ...
  start(env, { orm, "mail.store": mailStore, notification }) {
    const llmStore = reactive({
      ...
      async refreshThreadsAndSelect(threadId) {
        // Read the newly created thread data and insert into mailStore
        const threadData = await orm.read("llm.thread", [threadId], ["name", "write_date", "provider_id", "model_id"]);
        if (threadData && threadData.length) {
          const td = threadData[0];
          mailStore.Thread.insert({
            id: threadId,
            model: "llm.thread",
            name: td.name,
            write_date: td.write_date,
