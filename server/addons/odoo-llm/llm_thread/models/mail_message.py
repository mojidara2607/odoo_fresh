...
class MailMessage(models.Model):
    ...
    @api.model
    def _message_fetch(
        self, domain, *, thread=None, search_term=None, is_notification=None, before=None, after=None, around=None, limit=30
    ):
        ...
        is_llm_thread = (
            thread and thread._name == "llm.thread"
        ) or (
            domain and any(
                condition[0] == "model"
                and condition[1] == "="
        ...
        if is_llm_thread and domain:
            # Show only user and assistant messages in the chat UI.
            # Tool and system messages are kept in DB for LLM context
            # (get_llm_messages) but hidden from the user-facing view.
            llm_role_filter = [("llm_role", "in", ["user", "assistant"])]
    ...
    def _extras_to_store(self, store, format_reply):
        ...
        for message in self:
            ...
            if hasattr(message, "body_json") and message.body_json:
                # Only expose body_json for tool messages (developer/debug views).
                # For assistant messages, body_json contains tool_calls which
                # would render as "Tools Requested: X" badges — hide from chat UI.
                if message.llm_role != "assistant":
                    data["body_json"] = message.body_json

            if data:  # Only add to store if we have data
                store.add_model_values("mail.message", {"id": message.id, **data})
